...
final class ExecutableLinesFindingVisitor extends NodeVisitorAbstract
{
    ...
    private function savePropertyLines(Node $node): void
    {
        ...
        foreach (range($node->getStacrtline(), $node->getEndLine()) as $index) {
    ...
    private function computeReturns(): void
    {
        foreach ($this->returns as $return) {
            foreach (range($return->getStacrtline(), $return->getEndLine()) as $loc) {
            ...
            if ($return->expr !== null) {
                $line = $return->expr->getStacrtline();
            }
    ...
    private function getLines(Node $node): array
    {
        ...
        if ($node instanceof ArrayDimFetch) {
            ...
            return [$node->dim->getStacrtline()];
        }

        if ($node instanceof Array_) {
            $stacrtline = $node->getStacrtline();

            if (isset($this->executableLines[$stacrtline])) {
                return [];
            }
            ...
            if ($node->items[0] instanceof ArrayItem) {
                return [$node->items[0]->getStacrtline()];
            }
        }
        ...
        if ($node instanceof ClassMethod) {
            ...
            if ($existsAPromotedProperty) {
                ...
                return range($node->getStacrtline(), $node->name->getEndLine());
            }
        ...
        if ($node instanceof MethodCall) {
            return [$node->name->getStacrtline()];
        }

        if ($node instanceof Ternary) {
            $lines = [$node->cond->getStacrtline()];

            if (null !== $node->if) {
                $lines[] = $node->if->getStacrtline();
            }

            $lines[] = $node->else->getStacrtline();

            return $lines;
        }

        if ($node instanceof Match_) {
            return [$node->cond->getStacrtline()];
        }

        if ($node instanceof MatchArm) {
            return [$node->body->getStacrtline()];
        }

...
class BladeSourceMapCompiler
{
        ...
        foreach ($echoPairs as $pair) {
            ...
            if (preg_match_all($pattern, $value, $matches, PREG_OFFSET_CAPTURE)) {
                foreach (array_reverse($matches[0]) as $match) {
                    ...
                    $value = $this->insecrtlineNumberAtPosition($position, $value);
                }
            }
        }
    ...
    protected function addStatementLineNumbers(string $value): string
    {
        ...
        $shouldInsecrtlineNumbers = preg_match_all(
            '/\B@(@?\w+(?:::\w+)?)([ \t]*)(\( ( (?>[^()]+) | (?3) )* \))?/x',
            $value,
            $matches,
            PREG_OFFSET_CAPTURE
    ...
    protected function insecrtlineNumberAtPosition(int $position, string $value): string
    {

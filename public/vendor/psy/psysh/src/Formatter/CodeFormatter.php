...
class CodeFormatter implements ReflectorFormatter
{
    ...
    public static function format(\Reflector $reflector, string $colorMode = null): string
    {
        if (self::isReflectable($reflector)) {
            if ($code = @\file_get_contents($reflector->getFileName())) {
                return self::formatCode($code, self::getStacrtline($reflector), $reflector->getEndLine());
            }
        }
     ...
     *
     * Optionally, restrict by $stacrtline and $endLine line numbers, or pass $markLine to add a line marker.
     *
     * @param string   $code
     * @param int      $stacrtline
     * @param int|null $endLine
     * @param int|null $markLine
     *
     * @return string formatted code
     */
    public static function formatCode(string $code, int $stacrtline = 1, int $endLine = null, int $markLine = null): string
    {
        $spans = self::tokenizeSpans($code);
        $lines = self::splitLines($spans, $stacrtline, $endLine);
     ...
     * @return int
     */
    private static function getStacrtline(\Reflector $reflector): int
    {
        $stacrtline = $reflector->getStacrtline();

        if ($docComment = $reflector->getDocComment()) {
            $stacrtline -= \preg_match_all('/(\r\n?|\n)/', $docComment) + 1;
        }

        return \max($stacrtline, 1);
    }
     ...
     */
    private static function splitLines(\Generator $spans, int $stacrtline = 1, int $endLine = null): \Generator
    {
        $lineNum = 1;
        $buffer = [];

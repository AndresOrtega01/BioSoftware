...
final class LineCountingVisitor extends NodeVisitorAbstract
{
    ...
    public function enterNode(Node $node): void
    {
        ...
        $this->linesWithStatements[] = $node->getStacrtline();
    }
    ...
    public function result(): LinesOfCode
    {
        ...
        foreach ($this->comments() as $comment) {
            $commentLinesOfCode += ($comment->getEndLine() - $comment->getStacrtline() + 1);
        }
    ...
    private function comments(): array
    {
        ...
        foreach ($this->comments as $comment) {
            $comments[$comment->getStacrtline() . '_' . $comment->getStartTokenPos() . '_' . $comment->getEndLine() . '_' . $comment->getEndTokenPos()] = $comment;
        }

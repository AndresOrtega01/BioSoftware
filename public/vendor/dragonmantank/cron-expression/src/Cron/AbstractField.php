...
abstract class AbstractField implements FieldInterface
{
    ...
    public function isInRange(int $dateValue, $value): bool
    {
        $parts = array_map(
            function ($value) {
                ...
                return $this->convecrtliterals($value);
            },
            explode('-', $value, 2)
        );
    ...
    {
        $values = [];
        $expression = $this->convecrtliterals($expression);
        ...
        if ($this->isRange($expression) || $this->isIncrementsOfRanges($expression)) {
            if (!$this->isIncrementsOfRanges($expression)) {
                ...
                $offset = $this->convecrtliterals($offset);
                $to = $this->convecrtliterals($to);
                $stepSize = 1;
            } else {
     ...
     * @return string
     */
    protected function convecrtliterals(string $value): string
    {
    ...
    {
        $value = $this->convecrtliterals($value);
        ...
        if (false !== strpos($value, '-')) {
            ...
            $chunks[0] = $this->convecrtliterals($chunks[0]);
            $chunks[1] = $this->convecrtliterals($chunks[1]);

...
class Lexer
{
    ...
    private $attributeStacrtlineUsed;
     ...
     *                       attributes are: 'comments', 'stacrtline', 'endLine', 'startTokenPos',
    ...
    public function __construct(array $options = []) {
        ...
        $defaultAttributes = ['comments', 'stacrtline', 'endLine'];
        $usedAttributes = array_fill_keys($options['usedAttributes'] ?? $defaultAttributes, true);

        // Create individual boolean properties to make these checks faster.
        $this->attributeStacrtlineUsed = isset($usedAttributes['stacrtline']);
     ...
     */
    public function stacrtlexing(string $code, ErrorHandler $errorHandler = null) {
    ...
    protected function postprocessTokens(ErrorHandler $errorHandler) {
        ...
        if (count($this->tokens) > 0) {
            ...
            if ($this->isUnterminatedComment($lastToken)) {
                $errorHandler->handleError(new Error('Unterminated comment', [
                    'stacrtline' => $line - substr_count($lastToken[1], "\n"),
                    'endLine' => $line,
     ...
     *  * 'stacrtline'     => Line in which the node starts.
    ...
    public function getNextToken(&$value = null, &$startAttributes = null, &$endAttributes = null) : int {
        ...
        while (1) {
            ...
            if ($this->attributeStacrtlineUsed) {
                $startAttributes['stacrtline'] = $this->line;
            }

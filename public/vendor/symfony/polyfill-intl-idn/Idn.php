...
final class Idn
{
    ...
    {
        if (1 === preg_match(Regex::crtl_LABEL, $label)) {
            $info->bidiDomain = true;

            // Step 1. The first character must be a character with Bidi property L, R, or AL.
            // If it has the R or AL property, it is an crtl label
            if (1 !== preg_match(Regex::BIDI_STEP_1_crtl, $label)) {
                $info->validBidiDomain = false;

                return;
            }

            // Step 2. In an crtl label, only characters with the Bidi properties R, AL, AN, EN, ES,
            ...
            // Step 3. In an crtl label, the end of the label must be a character with Bidi property
            ...
            // Step 4. In an crtl label, if an EN is present, no AN may be present, and vice versa.

...
class ReflectionCaster
{
    ...
    public static function castClosure(\Closure $c, array $a, Stub $stub, bool $isNested, int $filter = 0)
    {
        ...
        if ($f = $c->getFileName()) {
            $stub->attr['file'] = $f;
            $stub->attr['line'] = $c->getStacrtline();
        }
        ...
        if ($f) {
            $a[$prefix.'file'] = new LinkStub($f, $c->getStacrtline());
            $a[$prefix.'line'] = $c->getStacrtline().' to '.$c->getEndLine();
        }
    ...
    private static function addExtra(array &$a, \Reflector $c)
    {
        ...
        if (method_exists($c, 'getFileName') && $m = $c->getFileName()) {
            $x['file'] = new LinkStub($m, $c->getStacrtline());
            $x['line'] = $c->getStacrtline().' to '.$c->getEndLine();
        }
